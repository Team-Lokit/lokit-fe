...
- [ ] 입력 필드가 정상적으로 포커스/입력 가능한가- [x] 기본 상태에서 border 없음, 포커스 시에만 표시
...
- [ ] hover/pressed 시 `blueWhite.bg8` 배경 디졸브 효과가 적용되는가- [x] 검색 결과 없을 때 안내 메시지가 표시되는가
- [x] 스크롤바가 숨겨져 있는가
...
> 로컬 `pnpm dev` 실행 후 실제 서비스에서 확인. E2E 테스트 도입 시 자동화 예정.
...
- [ ] 열릴 때 좌측에서 슬라이드 인 (`translateX(-100%) → translateX(0)`)이 동작하는가- [ ] 닫힐 때 좌측으로 슬라이드 아웃이 동작하는가- [ ] Backdrop이 fade in/out 되는가- [ ] transition이 `0.3s ease`로 부드럽게 동작하는가

### 2.3 검색 기능

- [ ] 검색어 입력 시 앨범 목록이 실시간 필터링 되는가- [x] 필터링 로직이 앨범 제목 포함 여부 기반인가
- [ ] 검색어 삭제 시 전체 앨범 목록이 복원되는가- [ ] 사이드바 닫았다 열면 검색어가 초기화되는가- [ ] 한글 검색이 정상 동작하는가 (조합 중 필터링 이슈 없음)

### 2.4 메뉴 동작

- [ ] "탐색" 클릭 시 탐색 페이지로 이동하는가- [ ] "새 앨범" 클릭 시 앨범 추가 모달이 열리는가

### 2.5 앨범 리스트 동작

- [ ] 전체사진 앨범 클릭 시 해당 앨범 상세(격자뷰)로 이동하는가- [ ] 사용자 앨범 클릭 시 해당 앨범 상세(`/album/:id`)로 이동하는가- [ ] 더보기(···) 메뉴 클릭 시 드롭다운(이름 변경/삭제)이 표시되는가- [ ] "앨범 이름 변경" 클릭 시 이름 변경 모달이 열리는가- [ ] "앨범 삭제" 클릭 시 삭제 확인 모달이 열리는가
...
- [ ] 지도보기 클릭 시 지도뷰로 전환되는가- [ ] 격자보기 클릭 시 격자뷰로 전환되는가- [x] 지도보기 아이콘이 `mapPin.svg`, 격자보기 아이콘이 `grid.svg`인가
...
- [ ] 앨범이 많을 때(20개+) AlbumList 영역 스크롤이 정상 동작하는가- [x] 앨범 이름이 매우 길 때 말줄임(ellipsis) 처리되는가 _(LongTitle 스토리로 확인)_
- [x] 사진 개수가 매우 클 때(10000+) 숫자 표시가 깨지지 않는가 _(99999로 확인)_
- [ ] 빠르게 열기/닫기 반복 시 애니메이션이 깨지지 않는가- [ ] 사이드바 열린 상태에서 브라우저 뒤로가기 시 적절히 처리되는가- [ ] Pending 사진(업로드 중)이 앨범 카운트에 정상 반영되는가
...
- [ ] 키보드 탐색(Tab)이 정상 동작하는가- [ ] 스크린리더가 사이드바 컨텐츠를 올바르게 읽는가
...
- [x] Sidebar의 nickname/dDay가 `useGetMyPage` API에서 가져오는가 (하드코딩 제거)
