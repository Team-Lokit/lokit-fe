...
- [ ] 입력 필드가 정상적으로 포커스/입력 가능한가 _(인터랙션 확인 필요)_
...
- [x] 전체사진 앨범이 최상단에 `blueWhite.bg8` 배경 강조 스타일로 표시되는가
...
- [ ] hover/pressed 시 `blueWhite.bg8` 배경 디졸브 효과가 적용되는가 _(인터랙션 확인 필요)_
...
> 섹션 2~3은 #2 (통합 PR)에서 검증 예정. 스토리북에서 확인 가능한 항목은 선행 체크.

### 2.1 사이드바 열기/닫기

- [ ] ≡ 햄버거 버튼 클릭 시 사이드바가 열리는가 _(#2에서 검증)_
- [ ] ✕ 버튼 클릭 시 사이드바가 닫히는가 _(인터랙션 확인 필요)_
- [ ] 사이드바 외부(backdrop) 클릭 시 닫히는가 _(인터랙션 확인 필요)_
- [ ] ESC 키 입력 시 닫히는가 _(인터랙션 확인 필요)_

### 2.2 열기/닫기 애니메이션

- [ ] 열릴 때 좌측에서 슬라이드 인 (`translateX(-100%) → translateX(0)`)이 동작하는가 _(인터랙션 확인 필요)_
- [ ] 닫힐 때 좌측으로 슬라이드 아웃이 동작하는가 _(인터랙션 확인 필요)_
- [ ] Backdrop이 fade in/out 되는가 _(인터랙션 확인 필요)_
- [ ] transition이 `0.3s ease`로 부드럽게 동작하는가 _(인터랙션 확인 필요)_

### 2.3 검색 기능

- [ ] 검색어 입력 시 앨범 목록이 실시간 필터링 되는가 _(인터랙션 확인 필요)_
- [x] 필터링 로직이 앨범 제목 포함 여부 기반인가
- [ ] 검색어 삭제 시 전체 앨범 목록이 복원되는가 _(인터랙션 확인 필요)_
- [ ] 사이드바 닫았다 열면 검색어가 초기화되는가 _(인터랙션 확인 필요)_
- [ ] 한글 검색이 정상 동작하는가 (조합 중 필터링 이슈 없음) _(인터랙션 확인 필요)_

### 2.4 메뉴 동작

- [ ] "탐색" 클릭 시 탐색 페이지로 이동하는가 _(#2에서 검증)_
- [ ] "새 앨범" 클릭 시 앨범 추가 모달이 열리는가 _(#2에서 검증)_

### 2.5 앨범 리스트 동작

- [ ] 전체사진 앨범 클릭 시 해당 앨범 상세(격자뷰)로 이동하는가 _(#2에서 검증)_
- [ ] 사용자 앨범 클릭 시 해당 앨범 상세(`/album/:id`)로 이동하는가 _(#2에서 검증)_
- [ ] 더보기(···) 메뉴 클릭 시 드롭다운(이름 변경/삭제)이 표시되는가 _(#2에서 검증)_
- [ ] "앨범 이름 변경" 클릭 시 이름 변경 모달이 열리는가 _(#2에서 검증)_
- [ ] "앨범 삭제" 클릭 시 삭제 확인 모달이 열리는가 _(#2에서 검증)_

### 2.6 My 영역 동작

- [ ] My 영역 클릭 시 마이페이지로 이동하는가 _(#2에서 검증)_
...
- [ ] 지도보기 클릭 시 지도뷰로 전환되는가 _(인터랙션 확인 필요)_
- [ ] 격자보기 클릭 시 격자뷰로 전환되는가 _(인터랙션 확인 필요)_
...
> #2 (통합 PR)에서 검증 예정. BottomSheet 제거 후 기존 기능이 정상 동작하는지 확인
...
- [x] 앨범이 0개일 때 사이드바가 정상 표시되는가 (전체사진만 표시) _(EmptyAlbums 스토리로 확인)_
- [ ] 앨범이 많을 때(20개+) AlbumList 영역 스크롤이 정상 동작하는가 _(인터랙션 확인 필요)_
- [x] 앨범 이름이 매우 길 때 말줄임(ellipsis) 처리되는가 _(LongTitle 스토리로 확인)_
- [ ] 사진 개수가 매우 클 때(10000+) 숫자 표시가 깨지지 않는가
- [ ] 빠르게 열기/닫기 반복 시 애니메이션이 깨지지 않는가 _(인터랙션 확인 필요)_
- [ ] 사이드바 열린 상태에서 브라우저 뒤로가기 시 적절히 처리되는가 _(#2에서 검증)_
- [ ] Pending 사진(업로드 중)이 앨범 카운트에 정상 반영되는가 _(#2에서 검증)_
...
- [ ] 키보드 탐색(Tab)이 정상 동작하는가 _(인터랙션 확인 필요)_
...
- [x] 기존 `blueWhite.border10` 사용처에 사이드 이펙트가 없는가 (BottomSheet 제거 후 무관)
- [x] Sidebar에 사용된 모든 색상이 `theme.colors`에서 참조되는가 (하드코딩 없음)
- [x] Sidebar에 사용된 효과가 `theme.effects`에서 참조되는가
- [x] Sidebar 너비가 `theme.layout.sidebarWidth`를 참조하는가 (하드코딩 없음)
